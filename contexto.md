...
- **Alterações Arquiteturais ou UI:**
    ...
    - **Correção Motor Parser (Vazamento de Profissões / HH):** O laço de extração das métricas da Mão de Obra (HH) não tinha freio e continuava lendo tabelas das seções subsequentes (Seção 5 - Indicadores) caso a formatação da Seção 4 não batesse exatamente. A lógica foi aprimorada para fazer um "Break" exato quando encontra o rodapé `TOTAL M.O.` ou o cabeçalho de uma nova seção, impedindo lixo como índice SINAPI e TCPO de entrarem na lista de profissionais.
    - **Correção Formatação (Listagem e `**` fantasmas):** O Renderizador foi imunizado para ignorar os asteriscos duplos que chegavam sem texto (`** **` ou `▸ **`), eliminando os bullets que vazavam. O layout da composição minimizada também foi modernizado para exibir o Título Longo ao lado do código. O parser agora prioriza obrigatoriamente a leitura da property embutida `TÍTULO:` ao invés do H1 inicial (Ex: `# COMPOSIÇÃO - CIV-...`).
